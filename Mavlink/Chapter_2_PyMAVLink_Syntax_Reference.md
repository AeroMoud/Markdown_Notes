# MAVLink Chapter 2 — PyMAVLink Syntax & Patterns: Complete Reference
...
- [Pattern 14 — Position Commands](#pattern-14--position-commands)
...
**PyMAVLink** is the official Python library that lets you communicate with drones using the MAVLink protocol. It converts your Python function calls into the exact binary packets the drone understands, and parses incoming packets back into readable Python objects.

Without PyMAVLink, you would need to manually construct binary packets, calculate checksums, and handle low-level communication yourself. PyMAVLink abstracts all of that away so you can focus on *what* you want the drone to do.

The library is used in ground control stations, companion computers, autonomous mission scripts, and testing tools. It is the same underlying protocol used by MAVProxy, Mission Planner, and QGroundControl under the hood.

---

## Why it matters in autonomous vehicles

In real-world autonomous drone systems — delivery drones, inspection UAVs, aerial mapping platforms — the flight computer (running ArduPilot or PX4) needs to receive commands and send back sensor data. PyMAVLink is what the companion computer (a Raspberry Pi, Jetson Nano, or laptop) uses to talk to the flight controller.

Companies like Skydio, Zipline, and AgEagle use MAVLink-based communication in their architectures. ROS-based drone stacks also interface with flight controllers through MAVLink bridges.

If you get the syntax wrong, the drone might not receive your command, arm when it shouldn't, fail to upload the mission, or fly to the wrong coordinates. Understanding the exact calling patterns is therefore safety-critical knowledge.
...
# Give the connection a moment to establish before requesting heartbeat
time.sleep(1)
...
# After wait_heartbeat(), these are populated:
...
Every `command_long_send` **always** takes exactly 7 command-specific parameters. Unused ones must be `0`. Missing a parameter causes a Python `TypeError`.

```
command_long_send(sys, comp, cmd, conf, p1, p2, p3, p4, p5, p6, p7)
                                        ↑──────────── 7 params ──────↑
```

### Checking if a command was accepted

After sending a command, listen for a `COMMAND_ACK` message to confirm the drone accepted it.
...
Use `recv_match()` to wait for specific message types from the drone.
...
if gps:
    # Coordinates stored as integers — divide by 1e7 to get degrees
    lat = gps.lat / 1e7
    lon = gps.lon / 1e7
...
### Non-blocking receive (polling loop)
...
Once you have a message object, access its data as **attributes** (like `msg.lat`, not `msg['lat']`).
...
# GLOBAL_POSITION_INT — EKF-fused data — more accurate than GPS_RAW_INT
...
# STATUSTEXT — human-readable status messages from the drone
...
if msg:
    ...
    if msg_type == 'HEARTBEAT':
        ...
        print(f"Yaw = {msg.yaw * 180 / 3.14159:.1f} degrees")
```
...
# Set GUIDED mode
# param1 = 1: MAV_MODE_FLAG_CUSTOM_MODE_ENABLED (required flag)
# param2 = 4: ArduCopter's internal number for GUIDED mode
master.mav.command_long_send(
    master.target_system,
    master.target_component,
    mavutil.mavlink.MAV_CMD_DO_SET_MODE,
    0,
    1,   # param1: base_mode flag — must be 1 to signal a custom mode change
    4,   # param2: custom_mode = 4 (ArduCopter GUIDED)
...
**Critical:** Both `param1=1` and `param2=4` are required. If you send `param2=0`, the mode change silently fails — the drone stays in its current mode. See [Common Mistakes](#common-mistakes) for more detail.
...
After sending the mode change, poll HEARTBEAT to confirm it took effect before continuing:

```python
def wait_for_mode(master, custom_mode_num, timeout=10):
    """Block until HEARTBEAT confirms the drone is in the requested mode."""
    start = time.time()
    while time.time() - start < timeout:
        hb = master.recv_match(type='HEARTBEAT', blocking=True, timeout=1)
        if hb and hb.custom_mode == custom_mode_num:
            print(f"Mode confirmed: custom_mode={custom_mode_num}")
            return True
    print("Mode confirmation timed out")
    return False
```
...
## Pattern 8 — Arming and Disarming
...
def arm_drone(master, force=False):
    ...
    21196 is ArduPilot's force-arm magic number.
    """
...
Uploading a mission requires a strict back-and-forth protocol between you and the drone.
...
def upload_mission(master, waypoints):
    ...
    while items_sent < total_items:
        ...
        master.mav.mission_item_int_send(
            ...
            1 if seq == 0 else 0,   # current: 1 = first waypoint
            1,                       # autocontinue
...
def monitor_telemetry(master, duration_seconds=30):
    ...
    while time.time() - start < duration_seconds:
        ...
        if msg_type == 'GLOBAL_POSITION_INT':
            lat = msg.lat / 1e7
            lon = msg.lon / 1e7
            alt = msg.relative_alt / 1000  # mm to m
...
## Pattern 12 — Parameter Read and Write
...
def get_parameter(master, param_name, timeout=5):
    ...
    """
    # Parameter name must be encoded as bytes
...
def set_message_rate(master, message_id, rate_hz):
    ...
    master.mav.command_long_send(
        ...
        0,
        message_id,
        1e6 / rate_hz if rate_hz > 0 else -1,  # interval in microseconds
        0, 0, 0, 0, 0
    )
...
## Pattern 16 — Error Handling and Timeouts
...
def send_command_with_retry(master, command, params, retries=3, timeout=5):
    ...
    for attempt in range(retries):
        ...
        if ack is None:
            print(f"  No ACK received (attempt {attempt + 1})")
            continue
...
`wait_heartbeat()` times out or `target_system` stays at 0.
...
Python raises a `TypeError` about wrong number of arguments.
...
`param_request_read_send` raises a `TypeError`.
...
`AttributeError` when accessing fields of a `None` message.
...
Your altitude readout shows hundreds of metres instead of your actual height above ground.
...
pos = master.recv_match(type='GLOBAL_POSITION_INT', blocking=True)
alt = pos.relative_alt / 1000  # Metres above where you launched
```
...
ARM command is rejected with `DENIED` or `TEMPORARILY_REJECTED`.
...
### Mistake 7: Sending wrong custom_mode in MAV_CMD_DO_SET_MODE

Mode change silently fails — the drone stays in its current mode with no error.

```python
# Wrong — param2=0 means STABILIZE, not GUIDED
master.mav.command_long_send(
    master.target_system, master.target_component,
    mavutil.mavlink.MAV_CMD_DO_SET_MODE,
    0,
...
# Correct — custom_mode=4 is GUIDED in ArduCopter
master.mav.command_long_send(
    master.target_system, master.target_component,
    mavutil.mavlink.MAV_CMD_DO_SET_MODE,
    0,
...
### Mistake 8: Using `GPS_RAW_INT` lat/lon directly without dividing by 1e7

Your waypoints end up thousands of kilometres away from the intended location.

```python
# Wrong — using the raw integer directly as degrees
lat = gps.lat   # This is 355363261, NOT 35.5363261 degrees

# Correct
lat = gps.lat / 1e7  # = 35.5363261 degrees
```
...
**Answer 3:** Two bugs: (1) `'WPNAV_SPEED'` must be `b'WPNAV_SPEED'` (bytes, not string); (2) `msg` is not checked for `None` before accessing `.param_value` — it will crash on timeout.
